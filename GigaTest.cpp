...
void AsynchronousDownloader::initializeMultiHandle()
{
  ...
  curl_multi_setopt(curlMultiHandle, CURLMOPT_MAX_HOST_CONNECTIONS, maxHandlesInUse);
  curl_multi_setopt(curlMultiHandle, CURLMOPT_MAXCONNECTS, maxHandlesInUse);
}
...
void closesocket_callback(void *clientp, curl_socket_t item)
{
  ...
  if (AD->socketTimerMap.find(item) != AD->socketTimerMap.end()) {
    std::cout << "Closing socket " << item << ", default\n";
...
void closeSocketCallback(uv_timer_t* handle)
{
  ...
  if (AD->socketTimerMap.find(socket) != AD->socketTimerMap.end()) {
    std::cout << "Closing socket " << socket << ", because of timer\n";
    close(socket);
